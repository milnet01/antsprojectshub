...
// No inline scripts/styles are emitted, so 'self' is strict and safe. README images
// are rewritten to absolute https URLs at build time, so img-src needs https but not
// data:. media-src is 'self' because demo videos are self-hosted under /assets/video/ —
// no third-party embeds. NOTE: frame-ancestors is ignored when delivered via <meta>
// (header-only) — it is kept for completeness but provides no active clickjacking
// protection here.
const CSP = [
  "default-src 'self'",
  "script-src 'self'",
...
export function basePage({ title, description, canonical, ogImage, content, back = false, lightbox = false }) {
