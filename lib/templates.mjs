...
// Structured data for search engines. A JSON-LD block is data, never executed, so the
// CSP's ban on inline script does not apply to it. Every "<" is escaped so no value
// (a tagline, a release name) can close the element early.
