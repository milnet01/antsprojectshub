...
// Daily views of every blog post since `since` (YYYY-MM-DD, the oldest post's date), plus
// the first day GA recorded any view at all — a post older than that was published before
// tracking began, and its first week is unknown rather than zero. Returns { rows, firstDay }
// or { error }: separate from collectAnalytics so a failure here costs only the blog table.
export async function collectPostViews(propertyId, since) {
  if (!propertyId) return null;
  const range = { startDate: since, endDate: "today" };
  try {
    const [rows, days] = await Promise.all([
      report(propertyId, range, ["pagePath", "date"], ["screenPageViews"], {
        dimensionFilter: {
          filter: { fieldName: "pagePath", stringFilter: { matchType: "BEGINS_WITH", value: "/blog/" } },
        },
        limit: 100000,
      }),
      report(propertyId, range, ["date"], ["screenPageViews"], {
        orderBys: [{ dimension: { dimensionName: "date" } }],
      }),
    ]);
    const first = days.find((d) => d.screenPageViews > 0);
    return { rows, firstDay: first ? first.date : null };
  } catch (err) {
    return { error: err.message };
  }
}

// Download-button clicks (src/assets/analytics.js's download_click event) per page over the
// last `days`. The page path names the project (/p/<slug>.html), which needs no GA setup;
// the event's own project/platform parameters would need registering as custom dimensions
// first, and this key is read-only. Returns { rows } or { error }, like collectPostViews.
export async function collectDownloadClicks(propertyId, { days = 28 } = {}) {
  if (!propertyId) return null;
  try {
    const rows = await report(
      ...
      {
        dimensionFilter: {
          filter: { fieldName: "eventName", stringFilter: { matchType: "EXACT", value: "download_click" } },
        },
      }
    );
    return { rows, days };
