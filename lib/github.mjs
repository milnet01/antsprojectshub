...
// A repo file as text, straight from raw.githubusercontent.com rather than the contents
// API. Needed because that API refuses to inline a file over 1 MB: it answers 200 with
// `encoding: "none"` and an EMPTY `content`, which reads as "this repo has no such file"
// rather than as a failure. Ants Terminal's CHANGELOG.md is 1.29 MB, so its whole release
// history — 100 versions — silently had no notes to fall back to. raw has no size cap.
//
// Same token, so a rate limit or a private repo behaves the way the rest of the build
// does, and text-or-null for the same reason ghJson is body-or-null: every caller here
// wants a fallback, never an exception.
export async function ghRaw(repo, filePath) {
  try {
    const res = await fetch(`https://raw.githubusercontent.com/${repo}/HEAD/${filePath}`, {
      headers: {
        "User-Agent": "ants-projects-hub-build",
        ...(TOKEN ? { Authorization: `Bearer ${TOKEN}` } : {}),
      },
